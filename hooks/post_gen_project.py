...
if __name__ == '__main__':
    ...
    COMPONENTS_PATHS_FOR_DELETE = {
        "consumer": ("src/modules/amqp-broker","src/config/index.ts"),
        "storage": ("src/modules/storage",),
        "db": ("src/config/db",),
        "mailer": ("src/config/mailer.config.ts",),
    }
    components_for_delete = set(COMPONENTS_PATHS_FOR_DELETE.keys()).difference({{cookiecutter.components.split(',')}})
    ...
    for component in components_for_delete:
        ...
        for path in COMPONENTS_PATHS_FOR_DELETE[component]:
